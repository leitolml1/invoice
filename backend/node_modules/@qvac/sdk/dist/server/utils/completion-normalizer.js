import { parseToolCalls } from '../../server/utils/tools/index.js';
export const MAX_TOOL_FRAME_CHARS = 1_048_576;
const DSML_INVOKE_FRAMES = [' ', '\n', '\t', '\r'].map((sep) => ({
...
const BUILTIN_THINKING = { open: '<think>', close: '</think>' };
export function createCompletionNormalizer(config) {
    let seq = 0;
    let state = 'content';
    let rawText = '';
    const contentParts = [];
    ...
    const useBuiltinThinkTags = config.captureThinking && config.capabilities.thinkingFraming === 'thinkTags';
    const dialect = config.toolDialect ?? 'hermes';
    const dialectSpec = DIALECT_SPECS[dialect];
    const toolFrames = useToolFraming ? dialectSpec.toolFrames : [];
    const thinkingFrames = [
        ...(useBuiltinThinkTags ? [BUILTIN_THINKING] : []),
        ...(dialectSpec.thinkingFrames ?? [])
    ];
    const contentFrames = dialectSpec.contentFrames ?? [];
    const dropTokens = dialectSpec.dropTokens ?? [];
    const contentDropTokens = dialectSpec.contentDropTokens ?? [];
    const markerToEntry = new Map();
    const dropTokenSet = new Set();
    const contentMarkers = [];
    function registerFrames(frames, kind) {
        for (const f of frames) {
            if (markerToEntry.has(f.open))
                continue;
    ...
    for (const t of dropTokens) {
        if (markerToEntry.has(t) || dropTokenSet.has(t))
            continue;
        dropTokenSet.add(t);
    ...
    function emitContent(events, text) {
        let normalized = text;
    ...
    function emitThinking(events, text) {
        if (!text)
            return;
    ...
    function emitStreamedToolCall(events, call) {
        streamedToolCallKeys.add(getToolCallKey(call));
        events.push({ type: 'toolCall', seq: nextSeq(), call });
    }
    function emitFinalToolCall(events, call) {
        if (streamedToolCallKeys.has(getToolCallKey(call)))
            return;
        events.push({ type: 'toolCall', seq: nextSeq(), call });
    }
    // Length of the longest suffix of `buf` that is also a prefix of any marker.
    function trailingPartialMarkerLen(buf, markers) {
        if (markers.length === 0 || buf.length === 0)
            return 0;
    ...
    function findEarliestMarker(buf, markers) {
        let best = null;
        for (const m of markers) {
            const i = buf.indexOf(m);
            if (i < 0)
                continue;
            if (best === null || i < best.idx)
                best = { idx: i, marker: m };
        }
        return best;
    }
    function flushToolBuffer(events, closed) {
        ...
        let inner = raw;
        if (inner.startsWith(frame.open))
            inner = inner.slice(frame.open.length);
        ...
        for (const call of toolCalls)
            emitStreamedToolCall(events, call);
    ...
    function abandonToolFrame(events) {
    ...
    function processSegment(events, text) {
        ...
        while (buf.length > 0) {
            if (state === 'toolFraming') {
                ...
                buf = '';
                if (toolCallBuffer.length > MAX_TOOL_FRAME_CHARS)
                    abandonToolFrame(events);
                continue;
            }
