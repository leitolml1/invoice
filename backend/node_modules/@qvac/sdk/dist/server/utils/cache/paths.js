...
import { getConfiguredCacheDir } from '../../../server/bare/registry/config-registry.js';
import { getQvacPath } from '../../../server/utils/qvac-paths.js';
import { calculateFileChecksum } from '../../../server/utils/checksum.js';
import { validateAndJoinPath } from '../../../server/utils/path-security.js';
import { generateShortHash } from '../../../server/utils/formatting.js';
import { getServerLogger } from '../../../logging/index.js';
import { nowMs } from '../../../profiling/index.js';
import { resolveClearStorageTarget } from '../../../server/utils/clear-storage.js';
...
export function getCacheDir(subDir) {
    ...
    try {
        ...
        logger.error(`Error creating cache directory (${subDir}):`, error instanceof Error ? error.message : String(error));
    }
    return cacheDir;
}
export function getModelsCacheDir() {
    const configuredDir = getConfiguredCacheDir();
    try {
        fs.mkdirSync(configuredDir, { recursive: true });
    }
    catch (error) {
        logger.error(`Error creating models cache directory:`, error instanceof Error ? error.message : String(error));
    }
    return configuredDir;
}
export function getKVCacheDir() {
 ...
 */
export function getShardedModelCacheDir(hyperdriveKey) {
    ...
    try {
        ...
        logger.error(`Error creating sharded model cache directory:`, error instanceof Error ? error.message : String(error));
    }
    return shardDir;
}
/**
 ...
 */
export function getCompanionSetCacheDir(setKey) {
    const baseCache = getModelsCacheDir();
    ...
    try {
        ...
        logger.error(`Error creating companion set cache directory:`, error instanceof Error ? error.message : String(error));
    }
    return setDir;
}
/**
 ...
 */
export function getCompanionSetPath(setKey, targetName) {
 ...
 */
export function getSingleFileCachePath(registryPath) {
 ...
 */
export function getShardPath(hyperdriveKey, shardFilename) {
 ...
 */
export function getClearStorageTarget(modelPath) {
 ...
 */
export async function checkShardCompleteness(hyperdriveKey, shardFilenames, shardMetadata, onChecksumTimeMs) {
    ...
    for (let i = 0; i < shardFilenames.length; i++) {
        ...
        try {
            ...
            if (stats.size !== fileMeta.expectedSize) {
                logger.warn(`File ${i + 1} size mismatch: expected ${fileMeta.expectedSize}, got ${stats.size}`);
                invalidIndices.push(i);
                continue;
            }
            if (fileMeta.sha256Checksum) {
                ...
                if (actualChecksum !== fileMeta.sha256Checksum) {
                    logger.warn(`File ${i + 1} checksum mismatch for ${fileMeta.filename}. Expected: ${fileMeta.sha256Checksum}. Actual: ${actualChecksum}. Will re-download.`);
                    invalidIndices.push(i);
                }
            }
        }
