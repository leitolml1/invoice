import { ModelLoadFailedError, InvalidShardUrlPatternError } from '../../utils/errors-server.js';
import { generateShortHash } from './formatting.js';
import { validateAndJoinPath } from './path-security.js';
import { extractAndWriteTensorsFile } from './gguf-tensor-extractor.js';
import { promises as fsPromises } from 'bare-fs';
/**
 * Detect if model filename follows shard pattern (00001-of-0000x)
 */
export function detectShardedModel(filename) {
 ...
 */
export function generateShardFilenames(shardName) {
 ...
 */
export function parsePatternBasedShardUrl(shardUrl) {
 ...
 */
export async function extractTensorsFromShards(shardDir, shardFilename) {
 ...
 */
export async function checkAllShardsExist(shardDir, shardFilename) {
    const shardInfo = detectShardedModel(shardFilename);
    if (!shardInfo.isSharded) {
        return false;
    }
    const shardFilenames = generateShardFilenames(shardFilename);
    const allShardsExist = await Promise.all(shardFilenames.map((f) => fsPromises
        .access(validateAndJoinPath(shardDir, f))
        .then(() => true)
        .catch(() => false))).then((results) => results.every((exists) => exists));
 ...
 */
export async function validateShardedModelCache(shardDir, shardFilename) {
    const shardInfo = detectShardedModel(shardFilename);
    if (!shardInfo.isSharded || !shardInfo.baseFilename) {
        return false;
    }
