...
export function parseGemmaFormat(text, tools) {
    const toolCalls = [];
    const errors = [];
    let parsed;
    try {
...
export function parseLlamacppFormat(text, tools) {
    const toolCalls = [];
    const errors = [];
    let parsedItem;
    try {
...
export function parseGenericFormat(text, tools) {
    const toolCalls = [];
    const errors = [];
    ...
    while ((match = jsonObjectRegex.exec(text)) !== null) {
        matched = true;
        try {
            const objItem = JSON.parse(match[0]);
