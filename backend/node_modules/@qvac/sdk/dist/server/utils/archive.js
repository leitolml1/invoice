...
import { detectShardedModel, generateShardFilenames, extractTensorsFromShards, validateShardedModelCache } from './shard-utils.js';
import { ModelLoadFailedError, ArchiveExtractionFailedError, ArchiveUnsupportedTypeError, ArchiveMissingShardsError, DownloadCancelledError } from '../../utils/errors-server.js';
import { isPathWithinBase } from '../../server/utils/path-security.js';
import { getServerLogger } from '../../logging/index.js';
import { SUPPORTED_ARCHIVE_EXTENSIONS, filenameToArchiveTypeSchema } from '../../schemas/archive.js';
...
function isModelFile(entryName) {
...
async function isGzip(archivePath) {
    return new Promise((resolve, reject) => {
        ...
        stream.on('end', () => {
            ...
            const isGzipped = buffer[0] === GZIP_MAGIC_0 && buffer[1] === GZIP_MAGIC_1 && buffer[2] === GZIP_MAGIC_2;
            resolve(isGzipped);
        });
        stream.on('error', reject);
    });
}
async function extractArchive(archivePath, extractDir, signal) {
    ...
    const actualType = isGzipped ? 'tar.gz' : 'tar';
...
export async function extractTarStream(archivePath, extractDir, useGunzip, signal) {
...
export async function extractAndValidateShardedArchive(archivePath, extractDir, signal) {
    ...
    if (!shardedFile) {
        throw new ModelLoadFailedError(`No sharded model files found in archive. Expected pattern: *-00001-of-0000X.*`);
    }
    const shardedFileName = String(shardedFile);
    const shardInfo = detectShardedModel(shardedFileName);
    if (!shardInfo.baseFilename) {
        throw new ModelLoadFailedError(`Could not extract base filename from sharded model: ${shardedFileName}`);
    }
