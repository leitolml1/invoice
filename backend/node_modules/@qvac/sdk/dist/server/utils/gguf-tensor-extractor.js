...
import { validateAndJoinPath } from './path-security.js';
...
class NeedMoreDataError extends Error {
    currentOffset;
    bytesNeeded;
    constructor(currentOffset, bytesNeeded) {
...
function parseGGUFHeader(buffer) {
    ...
    function skipValue(valueType) {
        ...
        if (valueType === GGUF_TYPE.ARRAY) {
            ...
            if (arrayLen > LIMITS.MAX_ARRAY_LEN) {
                throw new ModelLoadFailedError(`Array length ${arrayLen} exceeds limit ${LIMITS.MAX_ARRAY_LEN}`);
            }
    ...
    if (magicLE === GGUF_MAGIC_LE) {
        ...
        throw new ModelLoadFailedError(`Invalid GGUF file - magic number mismatch (got 0x${magicLE.toString(16)})`);
    }
    offset += 4;
    // Version
    ...
    if (tensorCount > LIMITS.MAX_TENSOR_COUNT) {
        throw new ModelLoadFailedError(`Tensor count ${tensorCount} exceeds limit ${LIMITS.MAX_TENSOR_COUNT}`);
    }
...
async function extractTensorNamesFromHeader(filePath) {
                ...
                try {
                    ...
                    if (error instanceof NeedMoreDataError) {
                        ...
                        if (totalRead >= LIMITS.MAX_HEADER_SIZE) {
                            throw new ModelLoadFailedError(`GGUF header exceeds ${LIMITS.MAX_HEADER_SIZE / (1024 * 1024 * 1024)}GB limit`);
                        }
                        continue;
                    }
                    throw error;
 ...
 */
export async function extractAndWriteTensorsFile(shardDir, shardFilenames, baseFilename) {
    ...
    if (failedShards.length > 0) {
        throw new ModelLoadFailedError(`Failed to extract tensors from ${failedShards.length}/${shardFilenames.length} shards: ${failedShards.join(', ')}`);
    }
    ...
    logger.info(`Tensors file created: ${tensorsFilePath} (${tensorNames.length} tensors from ${shardFilenames.length} shards)`);
    return tensorsFilePath;
}
/**
 ...
 */
export async function hasValidGGUFHeader(filePath) {
    try {
        const tensorNames = await extractTensorNamesFromHeader(filePath);
        return tensorNames.length > 0;
    }
    catch (error) {
        logger.warn(`⚠️ GGUF header validation failed: ${error instanceof Error ? error.message : String(error)}`);
        return false;
    }
}
