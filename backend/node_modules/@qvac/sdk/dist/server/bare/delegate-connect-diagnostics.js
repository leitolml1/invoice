...
export async function ensureDhtBootstrapped(dht, timeout) {
    if (dht.bootstrapped)
        return;
    const cap = timeout === undefined ? DHT_BOOTSTRAP_WAIT_CAP_MS : Math.min(timeout, DHT_BOOTSTRAP_WAIT_CAP_MS);
    try {
        await withTimeout(dht.fullyBootstrapped(), cap);
    }
    catch (error) {
        logger.warn(`DHT not bootstrapped within ${cap}ms before delegated connect; attempting anyway`, { error });
    }
}
...
export function describeConnectFailure(error, publicKey, relayCount) {
    const code = typeof error === 'object' && error !== null && 'code' in error ? String(error.code) : undefined;
    const relayNote = relayCount > 0 ? `${relayCount} swarm relay(s) configured` : 'no swarm relays configured';
    switch (code) {
        case 'PEER_NOT_FOUND':
