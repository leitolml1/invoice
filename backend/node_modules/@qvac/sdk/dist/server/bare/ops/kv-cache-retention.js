...
import { getKVCacheDir } from '../../../server/utils/index.js';
...
function getAutoCacheMarkerPath(cacheKey) {
    return path.join(getKVCacheDir(), `${AUTO_CACHE_MARKER_PREFIX}${cacheKey}`);
}
export async function markAutoCacheKey(cacheKey) {
    try {
        await fsPromises.writeFile(getAutoCacheMarkerPath(cacheKey), '');
    }
    catch (error) {
        logger.warn(`[kv-cache] Failed to write auto-cache retention marker. key=${cacheKey} error=${error instanceof Error ? error.message : String(error)}`);
    }
}
export async function removeAutoCacheMarker(cacheKey) {
    try {
...
export async function removeAutoCacheMarkerIfMissing(cacheKey) {
    try {
...
export function isCachePathWithinDirectory(directoryPath, cachePath) {
...
async function inspectAutoCacheEntry(cacheKey, activeCachePaths) {
    ...
    return {
        ...
        size,
        lastModifiedMs,
        active: activeCachePaths.some((cachePath) => isCachePathWithinDirectory(directoryPath, cachePath))
    };
}
export async function planAutoCacheEvictions(options) {
    ...
    for (const entry of entries) {
        if (entry.active)
            continue;
        const expired = entry.lastModifiedMs === 0 ||
