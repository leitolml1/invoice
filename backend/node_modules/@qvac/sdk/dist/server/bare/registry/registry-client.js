...
import { getCacheDir } from '../../../server/utils/cache/paths.js';
...
import { DEFAULT_REGISTRY_CORE_KEY } from '../../../constants/index.js';
...
async function initRegistryClient() {
    let lastError;
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            if (isFdLockError(error) && attempt < MAX_RETRIES) {
                ...
                if (remaining <= 0) {
                    logger.warn(`Registry client fd-lock failed after ${MAX_TOTAL_WAIT_MS}ms wait budget exhausted (attempt ${attempt}/${MAX_RETRIES})`);
                    throw error;
                }
                const backoff = Math.min(BASE_DELAY_MS * Math.pow(2, attempt - 1), remaining);
                logger.warn(`Registry client fd-lock failed (attempt ${attempt}/${MAX_RETRIES}), retrying in ${backoff}ms...`);
                await delay(backoff);
                continue;
            }
            throw error;
...
export async function closeRegistryClient() {
    if (!registryClient)
        return;
    ...
    try {
        ...
        logger.error('❌ Error closing registry client:', error instanceof Error ? error.message : String(error));
    }
    finally {
        if (corestore)
            unregisterCorestore(corestore);
        if (hyperswarm)
            unregisterSwarm(hyperswarm);
    }
}
export function hasActiveRegistryClient() {
