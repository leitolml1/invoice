import { definePlugin, defineHandler, ocrStreamRequestSchema, ocrStreamResponseSchema, ModelType, ocrConfigSchema, ADDON_OCR } from '../../../../schemas/index.js';
...
import { ocr } from '../../../../server/bare/plugins/ggml-ocr/ops/ocr-stream.js';
import { attachModelExecutionMs } from '../../../../profiling/model-execution.js';
import { resolveOcrConfig } from '../../../../server/bare/plugins/ggml-ocr/resolve-config.js';
function createOCRModel(modelId, detectorPath, recognizerPath, ocrConfig) {
...
export const ocrPlugin = definePlugin({
    ...
    createModel(params) {
        const ocrConfig = (params.modelConfig ?? {});
        const detectorModelPath = params.artifacts?.['detectorModelPath'];
        if (!detectorModelPath) {
            throw new ModelLoadFailedError('Detector model path missing. Ensure detectorModelSrc is provided in modelConfig.');
        }
        const model = createOCRModel(params.modelId, detectorModelPath, params.modelPath, // recognizerPath
        ocrConfig);
        return { model };
    },
    handlers: {
