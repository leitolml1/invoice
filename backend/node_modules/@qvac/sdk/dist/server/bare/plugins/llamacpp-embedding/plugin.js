...
import { definePlugin, defineHandler, embedRequestSchema, embedResponseSchema, ModelType, embedConfigBaseSchema, ADDON_EMBEDDING } from '../../../../schemas/index.js';
import { createStreamLogger, registerAddonLogger, getServerLogger } from '../../../../logging/index.js';
import { expandGGUFIntoShards } from '../../../../server/utils/index.js';
import { embed } from '../../../../server/bare/ops/embed.js';
import { forwardModelExecution } from '../../../../profiling/model-execution.js';
...
function transformEmbedConfig(embedConfig) {
    const config = {
        device: embedConfig.device,
        gpu_layers: `${embedConfig.gpuLayers}`,
...
function createEmbeddingsModel(modelId, modelPath, embedConfig) {
    ...
    if (isMobile()) {
        ...
        if (stripped.length > 0) {
            getServerLogger().warn(`[${ModelType.llamacppEmbedding}:${modelId}] Multi-GPU parameters (${stripped.join(', ')}) are not supported on mobile (single-GPU device) — removing from config; model will load with single-GPU defaults`);
        }
    }
...
export const embeddingsPlugin = definePlugin({
    ...
    createModel(params) {
        const embedConfig = (params.modelConfig ?? {});
