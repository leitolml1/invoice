...
function asUpscalerModel(model, modelId) {
...
export async function* upscale(request) {
    const model = asUpscalerModel(getModel(request.modelId), request.modelId);
    const response = await model.upscale(Buffer.from(request.image, 'base64'), request.repeats === undefined ? undefined : { repeats: request.repeats });
