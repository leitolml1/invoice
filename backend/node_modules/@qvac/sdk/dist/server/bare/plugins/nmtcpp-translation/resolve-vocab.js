import { BERGAMOT_CJK_LANG_PAIRS } from '../../../../schemas/index.js';
import { ModelLoadFailedError } from '../../../../utils/errors-server.js';
export function buildBergamotVocabSources(basePath, langPair) {
...
export function deriveBergamotVocabSources(modelSrc) {
...
export function deriveBergamotRegistryVocabSources(modelSrc) {
 ...
 */
export async function resolveBergamotVocab(nmtConfig, ctx, srcVocabSrc, dstVocabSrc, pivotModel) {
    let srcSrc = srcVocabSrc;
    let dstSrc = dstVocabSrc;
    if (!srcSrc || !dstSrc) {
    ...
    if (!srcSrc || !dstSrc) {
        throw new ModelLoadFailedError('Bergamot requires srcVocabSrc and dstVocabSrc. Provide them in modelConfig or use a pear:// or registry:// model source for auto-derivation.');
    }
    if (!pivotModel) {
    ...
    let pivotSrcSrc = pivotModel.srcVocabSrc;
    let pivotDstSrc = pivotModel.dstVocabSrc;
    ...
    if (!pivotSrcSrc || !pivotDstSrc) {
        throw new ModelLoadFailedError('Bergamot pivot model requires srcVocabSrc and dstVocabSrc. Provide them in modelConfig or use a pear:// or registry:// model source for auto-derivation.');
    }
    const modelIsRegistry = ctx.modelSrc.startsWith('registry://');
