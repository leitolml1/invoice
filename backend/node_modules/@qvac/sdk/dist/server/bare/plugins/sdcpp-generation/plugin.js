...
import { definePlugin, defineHandler, sdcppConfigSchema, diffusionRequestSchema, diffusionStreamResponseSchema, videoRequestSchema, videoStreamResponseSchema, upscaleRequestSchema, upscaleStreamResponseSchema, ModelType, ADDON_DIFFUSION } from '../../../../schemas/index.js';
import { createStreamLogger, registerAddonLogger, getServerLogger } from '../../../../logging/index.js';
import { ModelLoadFailedError } from '../../../../utils/errors-server.js';
import { isMobile } from '../../../../server/bare/registry/runtime-context-registry.js';
import { stripMultiGpuKeys } from '../../../../server/utils/multi-gpu-mobile.js';
...
export const diffusionPlugin = definePlugin({
    ...
    async resolveConfig(cfg, ctx) {
        ...
        const { clipLModelSrc, clipGModelSrc, clipVisionModelSrc, t5XxlModelSrc, llmModelSrc, vaeModelSrc, highNoiseDiffusionModelSrc, uncondModelSrc, audioVaeModelSrc, embeddingsConnectorsModelSrc, upscaler, ...rest } = cfg;
        ...
        const [clipLModelPath, clipGModelPath, clipVisionModelPath, t5XxlModelPath, llmModelPath, vaeModelPath, highNoiseDiffusionModelPath, uncondModelPath, audioVaeModelPath, embeddingsConnectorsModelPath, esrganModelPath] = await Promise.all([
    ...
    createModel(params) {
        ...
        if (isMobile()) {
            ...
            if (stripped.length > 0) {
                getServerLogger().warn(`[${ModelType.sdcppGeneration}:${modelId}] Multi-GPU parameters (${stripped.join(', ')}) are not supported on mobile (single-GPU device) — removing from config; model will load with single-GPU defaults`);
            }
        }
