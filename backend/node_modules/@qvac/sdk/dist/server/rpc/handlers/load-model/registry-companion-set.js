...
import { getModelsCacheDir, getCompanionSetPath, getCompanionSetCacheDir, getSingleFileCachePath, generateShortHash, calculatePercentage } from '../../../../server/utils/index.js';
import { measureChecksum } from '../../../../server/utils/checksum.js';
import { validateAndJoinPath } from '../../../../server/utils/path-security.js';
import { buildBlobBinding, downloadSingleFileFromRegistry, validateCachedFile } from './registry-download-utils.js';
import { DownloadCancelledError, RegistryDownloadFailedError } from '../../../../utils/errors-server.js';
 ...
 */
export async function downloadCompanionSetFromRegistry(options) {
    const { companionSet, downloadKey, allowLegacyFlatCache = false, progressCallback, signal, hooks, shouldClearCache } = options;
    ...
    for (const file of files) {
        ...
        const cached = await validateCachedFile(filePath, file.targetName, file.expectedSize, file.sha256Checksum, hooks);
        if (cached) {
    ...
    try {
        for (let i = 0; i < files.length; i++) {
            ...
            await downloadSingleFileFromRegistry(file.registryPath, file.registrySource, filePath, file.targetName, downloadKey, file.expectedSize, file.sha256Checksum, fileProgressCallback, signal, blobBinding, hooks);
            overallDownloaded += file.expectedSize;
        }
    }
    catch (error) {
...
function emitFinalProgress(progressCallback, downloadKey, setKey, files) {
    if (!progressCallback)
        return;
    const overallTotal = files.reduce((sum, f) => sum + f.expectedSize, 0);
...
function validateCompanionSetMetadata(files, primaryKey) {
    ...
    for (const file of files) {
        if (targetNames.has(file.targetName)) {
            throw new RegistryDownloadFailedError(`Companion set has duplicate targetName "${file.targetName}"`);
        }
        targetNames.add(file.targetName);
    }
}
function isLegacyOnnxSet(files) {
 ...
 */
async function checkLegacyOnnxCache(primaryRegistryPath, files, hooks) {
...
function getLegacyOnnxPath(cacheKey, filename) {
 ...
 */
async function checkLegacyFlatCache(files, primaryKey, hooks) {
    let primaryPath = null;
    for (const file of files) {
