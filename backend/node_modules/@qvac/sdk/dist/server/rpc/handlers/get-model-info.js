import { models } from '../../../models/registry/models.js';
import { getAllModelIds, getModelEntry } from '../../../server/bare/registry/model-registry.js';
import { generateShortHash } from '../../../server/utils/index.js';
import { promises as fsPromises } from 'bare-fs';
import { getShardPath, getModelsCacheDir, getSingleFileCachePath } from '../../../server/utils/cache/paths.js';
import { validateAndJoinPath } from '../../../server/utils/path-security.js';
import { ModelNotFoundError } from '../../../utils/errors-server.js';
export async function handleGetModelInfo(request) {
    const { name } = request;
    const catalogEntry = models.find((m) => m.name === name);
    if (!catalogEntry) {
        throw new ModelNotFoundError(`${name}" not found in catalog. Use model names from the catalog (e.g., "WHISPER_TINY", "EMBEDDINGGEMMA_300M_Q4_0")`);
    }
    const cacheKey = generateShortHash(catalogEntry.registryPath);
    const cacheStatus = catalogEntry.shardMetadata && catalogEntry.shardMetadata.length > 0
        ? await handleShardedModel(cacheKey, catalogEntry.shardMetadata)
        : catalogEntry.companionSet
            ? await handleCompanionSetModel(catalogEntry.companionSet, catalogEntry.registryPath, catalogEntry.addon)
            : await handleSingleFileModel(catalogEntry.registryPath, catalogEntry.expectedSize, catalogEntry.sha256Checksum);
...
async function handleShardedModel(cacheKey, shardMetadata) {
...
async function handleCompanionSetModel(companionSet, primaryRegistryPath, addon) {
    ...
    const canonicalPaths = files.map((f) => validateAndJoinPath(baseCache, 'sets', setKey, f.targetName));
    const canonicalResult = await checkCacheStatus(fileEntries, canonicalPaths, primaryKey);
    if (canonicalResult.isCached)
    ...
    if (isOnnxSet) {
        ...
        const legacyPaths = files.map((f) => validateAndJoinPath(baseCache, 'onnx', legacyCacheKey, f.targetName));
        const legacyResult = await checkCacheStatus(fileEntries, legacyPaths, primaryKey);
        if (legacyResult.isCached)
            return legacyResult;
    }
    else if (addon === 'nmt') {
        ...
        if (flatResult.isCached)
            return flatResult;
    }
    return canonicalResult;
}
async function checkCacheStatus(files, paths, primaryKey) {
    const cacheFiles = [];
    let allCached = true;
...
async function handleSingleFileModel(registryPath, expectedSize, sha256Checksum) {
