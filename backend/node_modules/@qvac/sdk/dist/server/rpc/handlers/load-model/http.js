...
import { getModelsCacheDir, getShardedModelCacheDir, generateShortHash, detectShardedModel, parsePatternBasedShardUrl, extractTensorsFromShards, calculatePercentage, isArchiveUrl, sanitizePathComponent, extractAndValidateShardedArchive, validateShardedModelCache, checkAllShardsExist, generateShardFilenames, hasValidGGUFHeader } from '../../../../server/utils/index.js';
import { getSDKConfig } from '../../../../server/bare/registry/config-registry.js';
import { getLifecycleState, onResume } from '../../../../server/bare/runtime-lifecycle.js';
import { createHttpDownloadKey, startOrJoinDownload, applyJoinedDownloadStats } from '../../../../server/rpc/handlers/load-model/download-manager.js';
import { DownloadCancelledError, HTTPError, NoResponseBodyError, PartialDownloadOfflineError, ResponseBodyNotReadableError } from '../../../../utils/errors-server.js';
...
async function validateCachedFile(modelPath, url, signal) {
    try {
        ...
        try {
            ...
            logger.warn(`⚠️ HEAD request failed: ${headError instanceof Error ? headError.message : String(headError)}`);
            ...
            if (localSize > 0) {
                logger.error(`❌ Offline with partial download (${localSize} bytes). Cannot resume without network.`);
        ...
        if (localSize !== expectedSize) {
            logger.info(`📥 Cached file size mismatch. Expected: ${expectedSize}, Found: ${localSize}. Re-downloading...`);
            return null;
        }
...
async function performHttpDownload(url, modelPath, downloadKey, progressCallback, signal) {
    if (signal?.aborted) {
 ...
 */
function isResumableTransferError(error) {
    if (error instanceof NoResponseBodyError ||
 ...
 */
async function waitForLifecycleActive(signal) {
    if (getLifecycleState() === 'active')
        return;
    const start = Date.now();
    while (getLifecycleState() !== 'active') {
        if (signal?.aborted)
            throw new DownloadCancelledError();
        if (Date.now() - start > LIFECYCLE_WAIT_MAX_MS)
            return;
 ...
 */
async function performHttpDownloadWithResume(url, modelPath, downloadKey, progressCallback, signal) {
    ...
    for (;;) {
        ...
        try {
            await performHttpDownload(url, modelPath, downloadKey, progressCallback, attemptController.signal);
            return;
        }
        catch (error) {
            ...
            attempt++;
            if (attempt > maxRetries || !isResumableTransferError(error))
                throw error;
            logger.warn(`⚠️ HTTP download interrupted (attempt ${attempt}/${maxRetries}), resuming from partial: ${error instanceof Error ? error.message : String(error)}`);
...
export async function downloadModelFromHttp(url, progressCallback, hooks) {
    ...
    const result = startOrJoinDownload(downloadKey, async (ctx) => {
        try {
            ...
            await performHttpDownloadWithResume(url, modelPath, downloadKey, ctx.broadcastProgress, ctx.signal);
            try {
...
async function downloadShardedModelFromHttp(shardUrl, progressCallback, hooks) {
    const config = getSDKConfig();
    ...
    const result = startOrJoinDownload(downloadKey, async (ctx) => {
        try {
            const shardStates = await Promise.all(shardInfos.map(async (shard, index) => {
            ...
            logger.info(`📏 Total size: ${overallTotal} bytes (${(overallTotal / 1024 / 1024).toFixed(2)} MB)`);
            const cacheChecks = await Promise.all(shardStates.map(async (state) => {
            ...
            await downloadShardsWithConcurrency(shardsToDownload, shardStates, concurrency, ctx.signal, downloadKey, overallTotal, ctx.broadcastProgress);
            ...
            logger.error('❌ Error during sharded download:', error instanceof Error ? error.message : String(error));
            if (error instanceof DownloadCancelledError) {
                if (ctx.shouldClearCache()) {
...
async function downloadShardedModelFromArchive(archiveUrl, progressCallback, hooks) {
    ...
    const result = startOrJoinDownload(downloadKey, async (ctx) => {
        ...
        async function downloadAndExtractArchive() {
            await performHttpDownloadWithResume(archiveUrl, archivePath, downloadKey, ctx.broadcastProgress, ctx.signal);
            logger.info(`✅ Archive downloaded, extracting to: ${extractDir}`);
            const firstShardPath = await extractAndValidateShardedArchive(archivePath, extractDir, ctx.signal);
            try {
...
async function downloadShardsWithConcurrency(shardsToDownload, allShards, concurrency, signal, downloadKey, overallTotal, progressCallback) {
    ...
    while (queue.length > 0 || inFlight.size > 0) {
        ...
        while (queue.length > 0 && inFlight.size < concurrency) {
            ...
            const downloadPromise = (async () => {
                ...
                await performHttpDownloadWithResume(state.shard.url, state.shardPath, downloadKey, (progress) => {
