...
import { measureChecksum, calculatePercentage } from '../../../../server/utils/index.js';
...
import { buildRegistryClientOptions } from './registry-client-options.js';
import { ChecksumValidationFailedError, DownloadCancelledError } from '../../../../utils/errors-server.js';
import { getServerLogger } from '../../../../logging/index.js';
const logger = getServerLogger();
export { DEFAULT_REGISTRY_STREAM_TIMEOUT_MS, buildRegistryClientOptions } from './registry-client-options.js';
 ...
 */
export async function validateCachedFile(modelPath, modelFileName, expectedSize, expectedChecksum, hooks) {
    try {
        ...
        logger.warn(`🗑️ Removing incomplete cached file (expected ${expectedSize}, got ${localSize}): ${modelPath}`);
 ...
 */
export async function downloadSingleFileFromRegistry(registryPath, registrySource, modelPath, modelFileName, downloadKey, expectedSize, expectedChecksum, progressCallback, signal, blobBinding, hooks) {
    if (signal?.aborted) {
    ...
    if (expectedSize && stats.size !== expectedSize) {
        await fsPromises.unlink(modelPath).catch(() => { });
        throw new ChecksumValidationFailedError(`${modelFileName}. File size mismatch: expected ${expectedSize}, got ${stats.size}`);
    }
    if (expectedChecksum && expectedChecksum.length === 64) {
        const checksum = await measureChecksum(modelPath, hooks);
        if (checksum !== expectedChecksum) {
            await fsPromises.unlink(modelPath);
            throw new ChecksumValidationFailedError(`${modelFileName}. Expected: ${expectedChecksum}. Actual: ${checksum}`);
        }
