import { REGISTRY_ERROR_CODES } from '../../../schemas/sdk-errors-registry.js';
import { getAddonFromEngine, resolveCanonicalEngine } from '../../../schemas/engine-addon-map.js';
import { getRegistryClient } from '../../../server/bare/registry/registry-client.js';
import { getServerLogger } from '../../../logging/index.js';
import { ModelRegistryQueryFailedError } from '../../../utils/errors-server.js';
...
function processRegistryModel(model) {
...
export async function handleModelRegistryList() {
    ...
    try {
        ...
        const qvacError = error;
        if (qvacError.code === REGISTRY_ERROR_CODES.FAILED_TO_CONNECT ||
        ...
        throw new ModelRegistryQueryFailedError(error instanceof Error ? error.message : 'Unknown error', error);
    }
}
export async function handleModelRegistrySearch(request) {
    ...
    try {
        ...
        if (request.filter) {
            ...
            models = models.filter((m) => m.name.toLowerCase().includes(filterLower) ||
...
export async function handleModelRegistryGetModel(request) {
    ...
    try {
        ...
        if (!rawModel) {
            throw new ModelRegistryQueryFailedError(`Model not found: ${request.registrySource}/${request.registryPath}`);
        }
        const model = processRegistryModel(rawModel);
        if (!model) {
            throw new ModelRegistryQueryFailedError(`Model has unknown engine "${rawModel.engine}": ${request.registryPath}`);
        }
