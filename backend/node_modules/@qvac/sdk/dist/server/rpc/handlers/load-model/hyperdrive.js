...
import { getQvacPath } from '../../../../server/utils/qvac-paths.js';
import { getModelsCacheDir, generateShortHash, detectShardedModel, getShardedModelCacheDir, getShardPath, checkShardCompleteness, measureChecksum, extractTensorsFromShards, calculatePercentage } from '../../../../server/utils/index.js';
import { getModelBySrc } from '../../../../models/registry/index.js';
import { createHyperdriveDownloadKey, startOrJoinDownload, applyJoinedDownloadStats } from '../../../../server/rpc/handlers/load-model/download-manager.js';
import { getSDKConfig } from '../../../../server/bare/registry/config-registry.js';
import { FileNotFoundError, ChecksumValidationFailedError, DownloadCancelledError, HyperdriveDownloadFailedError, ModelLoadFailedError, NoBlobFoundError } from '../../../../utils/errors-server.js';
import { getServerLogger } from '../../../../logging/index.js';
import { registerSwarm, unregisterSwarm, registerCorestore, unregisterCorestore } from '../../../../server/bare/runtime-lifecycle.js';
import { Buffer } from 'bare-buffer';
const logger = getServerLogger();
function getCorestoreDir(hyperdriveKey) {
...
async function setupHyperdrive(hyperdriveKey, corestoreDir) {
    ...
    const getRelays = () => {
        ...
        return relayPublicKeys.map((key) => Buffer.from(key, 'hex'));
    };
    const swarmOptions = {
...
export async function deleteCorestoreDirectory(corestoreDir) {
    ...
    try {
        ...
        logger.error(`Error deleting corestore directory: ${e instanceof Error ? e.message : String(e)}`);
    }
}
...
async function validateFileExistence(drive, file) {
    const { entry, targetPath } = await findFileInHyperdrive(drive, file);
    const { blobs, blocksDownloaded, totalBlocks, totalBytes } = await checkFileDownloaded(drive, entry);
    return {
        entry,
        blobs,
        targetPath,
...
async function validateCachedFile(modelPath, modelFileName, expectedSize, expectedChecksum, hooks) {
    try {
        ...
        if (localSize === expectedSize) {
            ...
            if (expectedChecksum && expectedChecksum.length === 64) {
                ...
                if (checksum !== expectedChecksum) {
                    throw new ChecksumValidationFailedError(`${modelFileName}. Expected: ${expectedChecksum}. Actual: ${checksum}. File may be corrupted`);
                }
            }
...
async function downloadSingleFileToFilesystem(modelPath, modelFileName, hyperdriveKey, corestoreDir, downloadKey, expectedChecksum, progressCallback, seed, signal, hooks, shouldClearCache) {
    ...
    try {
        const progressContext = progressCallback
            ? {
                downloadKey,
                callback: progressCallback
            }
            : undefined;
        await downloadAndValidateFile(setup.drive, modelFileName, modelPath, 0, // Size will be determined from drive
        expectedChecksum, progressContext, signal, hooks);
        ...
        logger.error('❌ Error during hyperdrive download:', error instanceof Error ? error.message : String(error));
        if (error instanceof DownloadCancelledError) {
            throw error;
        }
        throw new HyperdriveDownloadFailedError(`Download failed for ${modelFileName} from hyperdrive: ${hyperdriveKey}`, error);
    }
    finally {
...
async function downloadAndValidateFile(drive, sourceFileName, targetFilePath, expectedSize, expectedChecksum, progressContext, signal, hooks) {
    const { entry, blobs, targetPath, blocksDownloaded, totalBlocks, totalBytes } = await validateFileExistence(drive, sourceFileName);
    const blob = entry?.value?.blob;
    if (!blob) {
                ...
                if (progressContext.shardInfo) {
                    ...
                    progressContext.callback({
                        ...
                        shardInfo: {
                            ...
                            overallPercentage: progressContext.shardInfo.overallTotal > 0
    ...
    try {
        ...
        readStream = drive.createReadStream(targetPath);
        ...
        if (stats.size !== fileSizeToValidate) {
            throw new ChecksumValidationFailedError(`${path.basename(targetFilePath)}. File size mismatch: expected ${fileSizeToValidate}, got ${stats.size}`);
        }
        // Validate checksum
        if (expectedChecksum && expectedChecksum.length === 64) {
            ...
            if (checksum !== expectedChecksum) {
                ...
                throw new ChecksumValidationFailedError(`${path.basename(targetFilePath)}. Expected: ${expectedChecksum}. Actual: ${checksum}`);
            }
        }
...
async function downloadShardedFilesToFilesystem(hyperdriveKey, firstShardFileName, shardMetadata, progressCallback, seed, signal, hooks, shouldClearCache) {
    if (signal?.aborted) {
        throw new DownloadCancelledError();
    }
    ...
    logger.info(`Downloading sharded model with ${numberedShardCount} numbered shards + ${allFiles.length - numberedShardCount} companion files to ${shardDir}`);
    // Check which files need to be downloaded
    const invalidIndices = await checkShardCompleteness(hyperdriveKey, allFiles, shardMetadata, hooks?.addChecksumValidationTimeMs ? (ms) => hooks.addChecksumValidationTimeMs(ms) : undefined);
    ...
    try {
        ...
        for (let i = 0; i < allFiles.length; i++) {
            ...
            const progressContext = progressCallback
                ...
                : undefined;
            await downloadAndValidateFile(setup.drive, file, filePath, fileMeta.expectedSize, fileMeta.sha256Checksum, progressContext, signal, hooks);
            if (signal?.aborted) {
                throw new DownloadCancelledError();
            }
        ...
        logger.error('❌ Error during sharded hyperdrive download:', error instanceof Error ? error.message : String(error));
        // Preserve cancellation errors
        if (error instanceof DownloadCancelledError) {
            throw error;
        }
        throw new HyperdriveDownloadFailedError(`Sharded download failed for ${firstShardFileName} from hyperdrive: ${hyperdriveKey}`, error);
    }
    finally {
        signal?.removeEventListener('abort', cleanup);
        if (!seed) {
...
export async function downloadModelFromHyperdrive(hyperdriveKey, modelFileName, seed, progressCallback, hooks, expectedChecksum) {
    ...
    const result = startOrJoinDownload(downloadKey, async (ctx) => {
        if (shardInfo.isSharded && model?.shardMetadata) {
            const hooksWithCacheHit = {
                ...hooks,
                markCacheHit: () => {
                    hooks?.markCacheHit?.();
            ...
            return downloadShardedFilesToFilesystem(hyperdriveKey, modelFileName, model.shardMetadata, ctx.broadcastProgress, seed, ctx.signal, hooksWithCacheHit, ctx.shouldClearCache);
        }
        ...
        if (model) {
            const cachedPath = await validateCachedFile(modelPath, modelFileName, model.expectedSize, expectedChecksum || model.sha256Checksum, hooks);
            if (cachedPath) {
                hooks?.markCacheHit?.();
                ctx.setCacheHit(true);
        ...
        await downloadSingleFileToFilesystem(modelPath, modelFileName, hyperdriveKey, corestoreDir, downloadKey, checksumToValidate, ctx.broadcastProgress, seed, ctx.signal, hooks, ctx.shouldClearCache);
        return modelPath;
    }, progressCallback, hooks?.requestBinding);
