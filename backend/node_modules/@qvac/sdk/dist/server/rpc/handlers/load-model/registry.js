...
import { generateShortHash, detectShardedModel, getShardedModelCacheDir, getShardPath, extractTensorsFromShards, calculatePercentage } from '../../../../server/utils/index.js';
import { getSingleFileCachePath } from '../../../../server/utils/cache/paths.js';
import { getModelByPath } from '../../../../models/registry/index.js';
import { getRegistryClient } from '../../../../server/bare/registry/registry-client.js';
import { createRegistryDownloadKey, startOrJoinDownload, applyJoinedDownloadStats } from '../../../../server/rpc/handlers/load-model/download-manager.js';
import { buildBlobBinding, validateCachedFile, downloadSingleFileFromRegistry } from '../../../../server/rpc/handlers/load-model/registry-download-utils.js';
import { downloadCompanionSetFromRegistry } from '../../../../server/rpc/handlers/load-model/registry-companion-set.js';
import { DownloadCancelledError, ModelNotFoundError, RegistryDownloadFailedError } from '../../../../utils/errors-server.js';
import { getServerLogger } from '../../../../logging/index.js';
import { resolveRegistryDownloadMetadata } from './registry-metadata.js';
const logger = getServerLogger();
/**
 * Find all shards for a model using path prefix query.
 */
async function findModelShards(registryPath) {
    ...
    const sortedShards = shards
        ...
        checksum: s.blobBinding?.['sha256'] || s.sha256 || ''
    }));
 ...
 */
async function downloadShardedFilesFromRegistry(registryPath, registrySource, cacheKey, progressCallback, signal, localShardMetadata, hooks) {
    if (signal?.aborted) {
    ...
    for (let i = 0; i < shards.length; i++) {
        ...
        const cachedPath = await validateCachedFile(shardPath, shard.filename, shard.size, shard.checksum, hooks);
        if (cachedPath) {
        ...
        const shardProgressCallback = progressCallback
            ...
            : undefined;
        await downloadSingleFileFromRegistry(shard.path, shard.source, shardPath, shard.filename, downloadKey, shard.size, shard.checksum, shardProgressCallback, signal, shard.blobBinding, hooks);
        overallDownloaded += shard.size;
...
export async function downloadModelFromRegistry(registryPath, registrySource, progressCallback, expectedChecksum, hooks, explicitMetadata) {
    const downloadKey = createRegistryDownloadKey(registrySource, registryPath);
    ...
    const result = startOrJoinDownload(downloadKey, async (ctx) => {
        if (shardInfo.isSharded) {
            ...
            if (localShardMeta?.length) {
                ...
                for (const shard of localShardMeta) {
                    const shardPath = getShardPath(cacheKey, shard.filename);
                    const cached = await validateCachedFile(shardPath, shard.filename, shard.expectedSize, shard.sha256Checksum, hooks);
                    if (!cached) {
                        allCached = false;
                        break;
                    }
            ...
            try {
                return await downloadShardedFilesFromRegistry(registryPath, registrySource, cacheKey, ctx.broadcastProgress, ctx.signal, localShardMeta, hooks);
            }
            catch (error) {
        ...
        const { expectedSize, checksum } = resolveRegistryDownloadMetadata(modelMetadata, explicitMetadata, expectedChecksum);
        const cachedPath = await validateCachedFile(modelPath, filename, expectedSize, checksum, hooks);
        if (cachedPath) {
        ...
        ctx.setCacheHit(false);
        try {
            await downloadSingleFileFromRegistry(registryPath, registrySource, modelPath, filename, downloadKey, expectedSize, checksum, ctx.broadcastProgress, ctx.signal, blobBinding, hooks);
        }
        catch (error) {
            if (error instanceof DownloadCancelledError && ctx.shouldClearCache()) {
                try {
