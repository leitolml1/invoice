import { models, getModelByPath } from '../../../../models/registry/models.js';
import { hyperdriveUrlSchema, registryUrlSchema, SUPPORTED_ARCHIVE_EXTENSIONS } from '../../../../schemas/index.js';
import { getModelsCacheDir, getShardedModelCacheDir, generateShortHash, extractAndValidateShardedArchive } from '../../../../server/utils/index.js';
import { promises as fsPromises } from 'bare-fs';
import path from 'bare-path';
import { downloadModelFromHttp } from './http.js';
import { downloadModelFromHyperdrive } from './hyperdrive.js';
import { downloadModelFromRegistry } from './registry.js';
import { getExplicitRegistryMetadata } from './registry-metadata.js';
import { downloadModelFromHttpWithStats, downloadModelFromHyperdriveWithStats, downloadModelFromRegistryWithStats } from './download-stats.js';
import { InferenceCancelledError, ModelLoadFailedError, ModelNotFoundError, SeedingNotSupportedError } from '../../../../utils/errors-server.js';
import { validateAndJoinPath } from '../../../../server/utils/path-security.js';
import { getServerLogger } from '../../../../logging/index.js';
import { modelInputToSrcSchema } from '../../../../schemas/index.js';
...
async function resolveLocalOrCachedFile(modelIdOrPath) {
    ...
    if (isHexString) {
        throw new ModelLoadFailedError(`Direct hyperdrive keys not supported. Use hyperdriveKey parameter instead. Example: loadModel("model.gguf", "${modelIdOrPath}")`);
    }
    ...
    try {
        ...
        try {
            ...
            throw new ModelNotFoundError(`${modelIdOrPath}". Available models: ${availableModels.join(', ')}`);
        }
    }
}
function isDownloadResult(value) {
    return value !== null && typeof value === 'object' && 'path' in value;
}
function buildResult(pathOrResult, sourceType) {
...
async function resolveModelPathCore(modelSrc, progressCallback, seed, mode, signal, hooks) {
    if (signal?.aborted) {
    ...
    if (srcString.startsWith('registry://')) {
        ...
        const result = mode === 'stats'
            ? await downloadModelFromRegistryWithStats(registryPath, registrySource, progressCallback, expectedChecksum, hooks, explicitMetadata)
            : await downloadModelFromRegistry(registryPath, registrySource, progressCallback, expectedChecksum, hooks, explicitMetadata);
    ...
    if (hyperdriveKey) {
        ...
        const result = mode === 'stats'
            ? await downloadModelFromHyperdriveWithStats(hyperdriveKey, actualModelSrc, seed, progressCallback, hooks)
            : await downloadModelFromHyperdrive(hyperdriveKey, actualModelSrc, seed, progressCallback, hooks);
...
export async function resolveModelPath(modelSrc, progressCallback, seed, signal, hooks) {
    const result = await resolveModelPathCore(modelSrc, progressCallback, seed, 'base', signal, hooks);
    return result.path;
}
export async function resolveModelPathWithStats(modelSrc, progressCallback, seed, signal, hooks) {
