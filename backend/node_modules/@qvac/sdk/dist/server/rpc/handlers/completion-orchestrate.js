import { toolCallbackResultSchema } from '../../../schemas/completion-stream.js';
import { aggregateEvents } from '../../../utils/aggregate-events.js';
import { CompletionFailedError } from '../../../utils/errors-server.js';
import { dispatchPluginStream } from '../../../server/rpc/handlers/plugin-dispatch.js';
...
export class ToolResultReader {
    ...
    async pump() {
        try {
            ...
            if (!this.closed && this.buffer.trim())
    ...
    async waitFor(callId, timeoutMs = TOOL_RESULT_TIMEOUT_MS) {
    ...
    close() {
        ...
        this.results.clear();
        if (!this.failed)
            this.failWaiters(new CompletionFailedError('tool result stream closed'));
 ...
 */
export async function* orchestrateCompletion(request, runTurn, reader) {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
        ...
        for (const call of toolCalls) {
            ...
            const content = result.error !== undefined
...
export async function* handleCompletionOrchestrate(request, inputStream) {
    const reader = new ToolResultReader(inputStream);
    const runTurn = async function* (innerRequest) {
        yield* dispatchPluginStream(innerRequest.modelId, 'completionStream', innerRequest);
    };
    try {
