...
import { readModelExecutionMs } from '../../../profiling/model-execution.js';
import { readBackendDiagnostics } from './backend-diagnostics.js';
const metricsRegistry = new Map();
function extractDownloadStatsGauges(stats, gauges) {
...
export function registerOperationMetrics(config) {
    metricsRegistry.set(config.op, config);
}
export function buildOperationEvent(op, profileId, ts, executionMs, request, finalResponse, ttfb) {
    ...
    if (config.fromRequest && request) {
        ...
        if (extracted)
            Object.assign(gauges, extracted);
    }
...
registerOperationMetrics({
    ...
    fromFinalChunk: (res) => {
        ...
        if (modelExecMs !== undefined)
            gauges['modelExecutionTime'] = modelExecMs;
...
registerOperationMetrics({
    ...
    fromFinalChunk: (res) => {
        ...
        if (res.stats?.totalTokens !== undefined)
            gauges['totalTokens'] = res.stats.totalTokens;
        if (res.stats?.tokensPerSecond !== undefined)
            gauges['tokensPerSecond'] = res.stats.tokensPerSecond;
        if (res.stats?.timeToFirstToken !== undefined)
            gauges['timeToFirstToken'] = res.stats.timeToFirstToken;
        // NMT-specific
        if (res.stats?.decodeTime !== undefined)
            gauges['decodeTime'] = res.stats.decodeTime;
        if (res.stats?.encodeTime !== undefined)
            gauges['encodeTime'] = res.stats.encodeTime;
        // LLM-specific
        if (res.stats?.cacheTokens !== undefined)
            gauges['cacheTokens'] = res.stats.cacheTokens;
        return Object.keys(gauges).length > 0 ? gauges : undefined;
...
registerOperationMetrics({
    ...
    fromFinalChunk: (res) => {
        ...
        if (modelExecMs !== undefined)
            gauges['modelExecutionTime'] = modelExecMs;
        // Common stats
        if (res.stats?.audioDuration !== undefined)
            gauges['audioDuration'] = res.stats.audioDuration;
        if (res.stats?.realTimeFactor !== undefined)
            gauges['realTimeFactor'] = res.stats.realTimeFactor;
        if (res.stats?.tokensPerSecond !== undefined)
            gauges['tokensPerSecond'] = res.stats.tokensPerSecond;
        if (res.stats?.totalTokens !== undefined)
            gauges['totalTokens'] = res.stats.totalTokens;
        if (res.stats?.totalSegments !== undefined)
            gauges['totalSegments'] = res.stats.totalSegments;
        ...
        if (res.stats?.encoderTime !== undefined)
            gauges['encoderTime'] = res.stats.encoderTime;
        if (res.stats?.decoderTime !== undefined)
            gauges['decoderTime'] = res.stats.decoderTime;
        if (res.stats?.melSpecTime !== undefined)
            gauges['melSpecTime'] = res.stats.melSpecTime;
        return Object.keys(gauges).length > 0 ? gauges : undefined;
    }
});
registerOperationMetrics({
    ...
    fromFinalChunk: (res) => {
        ...
        if (res.stats?.audioDuration !== undefined)
            gauges['audioDuration'] = res.stats.audioDuration;
        if (res.stats?.totalSamples !== undefined)
            gauges['totalSamples'] = res.stats.totalSamples;
        return Object.keys(gauges).length > 0 ? gauges : undefined;
    }
});
...
registerOperationMetrics({
    ...
    fromFinalChunk: (res) => {
        ...
        if (res.stats.conditionerMs !== undefined)
            gauges['conditionerMs'] = res.stats.conditionerMs;
        if (res.stats.denoiseMs !== undefined)
            gauges['denoiseMs'] = res.stats.denoiseMs;
        if (res.stats.vaeMs !== undefined)
            gauges['vaeMs'] = res.stats.vaeMs;
        if (res.stats.postProcessMs !== undefined)
            gauges['postProcessMs'] = res.stats.postProcessMs;
        if (res.stats.stepsPerSecond !== undefined)
            gauges['stepsPerSecond'] = res.stats.stepsPerSecond;
...
registerOperationMetrics({
    ...
    getTags: (req) => {
        const tags = {};
        if (req.modelId)
            tags['modelId'] = req.modelId;
        if (req.handler)
            tags['handler'] = req.handler;
        return tags;
    }
});
registerOperationMetrics({
