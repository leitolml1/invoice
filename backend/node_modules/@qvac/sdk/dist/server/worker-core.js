...
import { createBareKitRPCServer, createIPCClient } from '../server/rpc/create-server.js';
import { destroySwarm } from '../server/bare/hyperswarm.js';
import { initEnv, getValidatedEnv } from '../server/env.js';
import { closeAllRagInstances } from '../server/bare/rag-hyperdb/index.js';
import { cleanupDownloads } from '../server/rpc/handlers/load-model/download-manager.js';
import { unloadAllModels } from '../server/bare/registry/model-registry.js';
import { closeRegistryClient } from '../server/bare/registry/registry-client.js';
import { clearAllLoggingStreams, startLogBuffering, stopLogBufferingWithTimeout } from '../server/bare/registry/logging-stream-registry.js';
import { clearAllAddonLoggers, getServerLogger, SDK_LOG_ID, SDK_ALL_LOG_ID } from '../logging/index.js';
import { clearPlugins } from '../server/plugins/index.js';
import { acquireWorkerLock, releaseWorkerLock } from '../server/utils/worker-lock.js';
import { nativeResourceCollectorDependencies } from '../server/bare/resources/native.js';
import { destroyWorkerResourceCollector, initializeWorkerResourceCollector } from '../server/bare/resources/worker-collector.js';
...
function scheduleForceExit() {
    const timer = setTimeout(() => {
        logger.error(`Bare.exit did not terminate the worker within ${FORCE_EXIT_GRACE_MS}ms — ` +
...
export function initializeWorkerCore() {
    if (coreInitialized) {
 ...
 */
export async function cleanupForTerminate() {
...
export async function shutdownBareDirectWorker(reason) {
    if (isShuttingDown)
        return;
    isShuttingDown = true;
    const messages = {
