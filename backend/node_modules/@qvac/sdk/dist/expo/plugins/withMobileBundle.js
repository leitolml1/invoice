...
import { bundleSdk, verifyBundle, hasErrors, formatVerifyBundleResult } from '../../commands/index.js';
import { CONFIG_CANDIDATES } from '../../client/config-loader/resolve-config.node.js';
import { resolveSDKPackageDir } from '../../expo/plugins/resolve-sdk-package-dir.js';
import { getProjectRootFromMod } from '../../expo/plugins/get-project-root.js';
import { findInAncestorNodeModules } from '../../expo/plugins/find-in-ancestor-node-modules.js';
import { BundleVerificationFailedError } from '../../utils/errors-client.js';
...
async function runVerifier(projectRoot, generatedBundle, configPath) {
    if (!configPath) {
        console.log('⚠️ QVAC: no qvac.config.* found — Bare runtime will be auto-detected ' +
...
async function runBundler(projectRoot, qvacSdkPath, configPath, deferredModules) {
 ...
 */
function patchBareKitLinkers(projectRoot, qvacSdkPath) {
    const bareKitPath = findInAncestorNodeModules(projectRoot, 'react-native-bare-kit');
    if (bareKitPath === null) {
        console.warn('⚠️ QVAC: react-native-bare-kit not found in any ancestor node_modules, ' +
