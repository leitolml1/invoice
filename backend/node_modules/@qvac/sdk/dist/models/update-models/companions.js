...
import { BERGAMOT_MODEL_RE } from '../../schemas/index.js';
/**
 ...
 */
export function groupCompanionSets(models) {
...
function groupOnnxCompanions(models, bySourcePath, companionKeys) {
    for (const model of models) {
        if (!model.registryPath.endsWith('.onnx'))
            continue;
...
function groupBergamotCompanions(models, bySourcePath, companionKeys) {
    for (const model of models) {
...
function groupBciCompanions(models, bySourcePath, companionKeys) {
    for (const model of models) {
        const filename = model.registryPath.split('/').pop();
        if (filename !== BCI_PRIMARY_FILENAME)
            continue;
...
const MECAB_REQUIRED_FILENAMES = ['dicrc', 'matrix.bin', 'mecabrc', 'sys.dic', 'unk.dic'];
const MECAB_IPADIC_DIR_SUFFIX = '/mecab-ipadic/';
function groupMecabCompanions(models, bySourcePath, companionKeys) {
    for (const model of models) {
        if (!model.registryPath.includes(MECAB_IPADIC_DIR_SUFFIX))
            continue;
        const lastSep = model.registryPath.lastIndexOf('/');
        const filename = lastSep >= 0 ? model.registryPath.slice(lastSep + 1) : model.registryPath;
        if (filename !== MECAB_PRIMARY_FILENAME)
            continue;
        const dirPrefix = lastSep >= 0 ? model.registryPath.slice(0, lastSep + 1) : '';
        const source = model.registrySource;
        const companions = findMecabCompanions(source, dirPrefix, bySourcePath);
        if (companions.length !== MECAB_REQUIRED_FILENAMES.length)
            continue;
...
function findBergamotCompanions(source, dirPrefix, langPair, bySourcePath) {
    const found = [];
...
function findMecabCompanions(source, dirPrefix, bySourcePath) {
    const found = [];
...
function sourceKey(source, path) {
    return `${source}:${path}`;
}
function findOnnxCompanionKey(source, primaryPath, bySourcePath) {
