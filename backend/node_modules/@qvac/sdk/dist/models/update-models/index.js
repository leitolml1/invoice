...
import { generateModelsFileContent } from './codegen.js';
import { assignNames, compareModels, createHistoryFile, loadCurrentModels, separateUpdates } from './history.js';
import { collectModels } from './registry.js';
...
async function checkOnly(nonBlocking = false, showDuplicates = false) {
    ...
    try {
        const result = await Promise.race([
            (async () => {
                ...
                remoteModels.sort((a, b) => a.addon.localeCompare(b.addon) || a.registryPath.localeCompare(b.registryPath));
...
async function updateModels(showDuplicates = false, noDedup = false) {
    ...
    models.sort((a, b) => a.addon.localeCompare(b.addon) || a.registryPath.localeCompare(b.registryPath));
    ...
    if (added.length > 0 || removed.length > 0) {
        ...
        if (historyFile) {
            const { added: trulyAdded, updated, removed: trulyRemoved } = separateUpdates(addedWithNames, removed);
            console.log(`📜 Created history file → ${historyFile}`);
            console.log(`   Added: ${trulyAdded.length}, Updated: ${updated.length}, Removed: ${trulyRemoved.length}`);
        }
    }
}
async function main() {
