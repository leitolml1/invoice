import { getAddonFromEngine } from '../../schemas/engine-addon-map.js';
import { detectShardedModel } from './shards.js';
...
export function generateExportName({ path: registryPath, engine, name: modelName, quantization, params, tags, usedNames }) {
...
function resolveCollision(exportName, quantization, usedNames) {
...
function generateBaseName(input) {
...
function generateAudioGenName({ filename }) {
    const modelName = cleanPart(filename.replace(/\.\w+$/, ''));
    return `AUDIOGEN_${modelName}`;
}
function generateVadSileroName({ filename }) {
...
function generateWhisperName({ filename, lowerFilename, quantization }) {
    ...
    if (modelSize)
        nameParts.push(modelSize);
    if (fileQuant)
        nameParts.push(fileQuant);
    return `WHISPER_${nameParts.map(cleanPart).join('_')}`;
}
function generateBciName({ filename, quantization }) {
...
function generateVadName({ filename, tagType, tagName }) {
    const versionMatch = filename.match(/v(\d+\.\d+\.\d+)/i);
    const version = versionMatch ? versionMatch[1] : '';
...
function generateNmtName(input) {
...
function generateNmtSalamandraName({ filename }) {
    const base = filename.replace(/\.\w+$/, '').replace(/-\d{5}-of-\d{5}/, '');
    return cleanPart(base);
}
function generateNmtIndictransName({ filename, quantization }) {
...
function generateNmtBergamotName({ filename }) {
...
function generateLlmName({ filename, tagType, tagName, modelName, quantization, params }) {
    const type = tagType || '';
    ...
    if (params) {
        ...
        if (familyMatch?.[1]) {
            ...
            if (cleanedCandidate.startsWith(cleanedTagCore) &&
    ...
    const paramsAlreadyIncluded = params !== '' && familyTokens[familyTokens.length - 1] === cleanPart(params);
    const nameParts = [familyName, paramsAlreadyIncluded ? '' : params, type, quantization].filter((p) => p && p !== '');
...
function generateEmbeddingsName({ tagName, modelName, params, quantization }) {
    const name = tagName || modelName || '';
    const nameParts = [name, params, quantization].filter((p) => p && p !== '');
    return nameParts.map(cleanPart).join('_');
}
function generateTtsName({ filename, tagName, modelName, tagExtra, tagType, quantization }) {
    const name = tagName || modelName || '';
...
function generateOcrName({ filename, tagName, modelName, tagExtra }) {
    const name = tagName || modelName || '';
    const language = tagExtra || '';
    let fileType = '';
...
function generateDiffusionName({ filename, modelName, quantization, params, tags }) {
...
function generateParakeetName({ filename, lowerPath, quantization }) {
    ...
    if (lower.endsWith('.gguf')) {
        ...
        const nameParts = [variant, paramsHint, speakerHint, versionHint, quantization].filter((p) => p && p !== '');
