...
function toPearConfigPath(pathFromAppRoot) {
...
function detectJsonNewline(raw) {
    return raw.includes('\r\n') ? '\r\n' : '\n';
}
function ensureObjectProp(obj, prop) {
...
function normalizePearWorkerPath(value) {
    if (typeof value !== 'string')
        return DEFAULT_PEAR_WORKER;
    let p = value.trim();
    if (!p)
        return DEFAULT_PEAR_WORKER;
    p = toPosixPath(p);
    if (/^[A-Za-z]:\//.test(p)) {
        throw new Error(`"pearWorker" must be a path inside the app root (got absolute path: ${JSON.stringify(value)})`);
    }
    if (p.startsWith('./'))
        p = p.slice(2);
    while (p.startsWith('/'))
    ...
    if (p === '.' || p === '..' || p.startsWith('../') || p.includes('/../')) {
        throw new Error(`"pearWorker" must be a path inside the app root (got: ${JSON.stringify(value)})`);
    }
    return p;
}
/**
...
function ensureArrayIncludes(existing, required) {
    const next = [...existing];
 ...
 */
function persistStageEntrypointsToPackageJson(appRoot, requiredEntrypoints) {
...
function findConfigFile(appRoot) {
...
async function loadConfig(configPath) {
    ...
    if (ext === '.ts') {
        throw new Error('TypeScript config not supported in Pear pre-hook. Use qvac.config.json or qvac.config.mjs.');
    }
...
async function discoverConfig(appRoot) {
...
function parseBuiltinSpecifier(specifier) {
...
function generatePearWorkerEntry(plugins, appWorkerPath) {
    const imports = [];
    const registrations = [];
    let varIndex = 0;
...
pipe.once('data', (data) => {
    void (async () => {
        let options = null;
        try {
            options = cenc.decode(cenc.any, data);
        }
        catch (err) {
            exitCode = 1;
            console.error(LOG_PREFIX, err);
