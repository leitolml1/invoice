...
import { RPCConnectionFailedError } from '../../utils/errors-client.js';
import { initializeConfig } from '../../client/init-hooks.js';
import { resolveConfig } from '../../client/config-loader/resolve-config.expo.js';
import { getClientLogger } from '../../logging/index.js';
import { getDeviceInfo } from '../../client/rpc/expo-device-info.js';
...
export async function getRPC() {
    ...
    rpcPromise = (async () => {
        ...
        try {
            ...
            mobileBundle = require('@qvac/sdk/worker.mobile.bundle');
        }
        catch (error) {
            const message = 'Failed to load mobile worker bundle. Make sure the QVAC Expo plugin is configured. ' +
        ...
        rpcInstance = new RPC(worklet.IPC, () => {
 ...
 */
async function sendShutdownMessage(rpc) {
    let timer;
    try {
        await Promise.race([
            (async () => {
            ...
            new Promise((_, reject) => {
                timer = setTimeout(() => reject(new Error(`Worker did not ack __shutdown__ within ${SHUTDOWN_RPC_TIMEOUT_MS}ms`)), SHUTDOWN_RPC_TIMEOUT_MS);
            })
        ]);
    }
    catch (error) {
        // Best-effort: log but don't block termination if cleanup fails.
        logger.warn(`⚠️ Pre-terminate worker cleanup failed: ${error instanceof Error ? error.message : String(error)}`);
    }
    finally {
        if (timer)
            clearTimeout(timer);
