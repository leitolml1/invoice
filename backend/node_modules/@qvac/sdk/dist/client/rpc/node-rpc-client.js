...
import { resolveConfig } from '../../client/config-loader/resolve-config.node.js';
...
function getDefaultWorkerPath() {
    ...
    const packagedUrl = hasAsset
        ? new URL(import.meta.asset('../../server/worker.js'))
    ...
    const devUrl = hasAsset
        ? new URL(import.meta.asset('../../dist/server/worker.js'))
...
function createWorkerStartupError(details, stderrTail) {
    ...
    rpcPromise = new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
            ...
            const cause = workerStderrTail
                ? createWorkerStartupError('Worker did not establish IPC before the RPC initialization timeout', workerStderrTail)
                : undefined;
            teardownFailedInit();
        ...
        ipcServer.listen(socketPath, () => {
            ...
            if (bareWorkerProc) {
                ...
                bareWorkerProc.on('close', (...args) => {
                    ...
                    clearTimeout(timer);
                    teardownFailedInit();
                    reject(new RPCInitTimeoutError(RPC_INIT_TIMEOUT_MS, createWorkerStartupError(`Worker process exited with code ${code}, signal ${exitSignal} before IPC connection was established`, workerStderrTail)));
                });
            }
        });
    });
