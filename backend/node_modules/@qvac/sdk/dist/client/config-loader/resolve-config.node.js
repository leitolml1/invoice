...
import { validateConfig, parseJsonConfig } from './config-utils.js';
import { ConfigFileInvalidError, ConfigFileParseFailedError } from '../../utils/errors-client.js';
...
async function loadJsConfig(filePath) {
    try {
        ...
        throw new ConfigFileParseFailedError(filePath, error instanceof Error ? error.message : String(error), error);
    }
}
async function loadTsConfig(filePath) {
    ...
    try {
        ...
        throw new ConfigFileInvalidError(filePath, 'Loading a TypeScript qvac.config.ts requires the optional peer dependency `tsx`. Install it as a devDependency, or use qvac.config.mjs / qvac.config.js / qvac.config.json instead.', error);
    }
    try {
        const tsxModule = (await import(tsxApiPath));
...
export async function loadConfigFromPath(configPath) {
    ...
    throw new ConfigFileInvalidError(configPath, `Unsupported config format: ${ext}. Use .json, .js, .mjs, or .ts`);
}
async function findConfigFileInDir(searchDir) {
 ...
 */
export async function resolveConfigFileInProject(projectRoot, explicitPath) {
    if (explicitPath) {
        ...
        throw new ConfigFileInvalidError(absPath, 'Config file not found at explicit path');
    }
 ...
 */
export async function resolveConfigForProject(projectRoot, explicitPath) {
    const configPath = await resolveConfigFileInProject(projectRoot, explicitPath);
    if (!configPath) {
...
async function findPackagedConfig() {
