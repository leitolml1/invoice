...
const caption = process.argv[2] ?? 'Lo-fi hip hop with mellow piano, soft drums, and a warm bass line';
const outputPath = process.argv[3] ?? 'audiogen-output.wav';
let modelId;
const lastLoggedPercentageByDownload = new Map();
...
try {
    ...
    modelId = await loadModel({
        ...
        onProgress: (progress) => {
            ...
            const line = `▸ Downloading ${label}: ${progress.percentage.toFixed(0)}% ` +
    ...
    console.log(`▸ Generated ${samplesPerChannel} samples per channel at ` +
...
function getDownloadLabel(downloadKey) {
