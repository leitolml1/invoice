...
import { deduplicateAddons, readAddonPackageJson } from '../../commands/verify/addon-source.js';
export class InvalidNodeModulesSourceError extends Error {
    nodeModulesPath;
    constructor(nodeModulesPath, cause) {
        const reason = cause instanceof Error ? cause.message : String(cause);
        super(`node_modules at ${nodeModulesPath} could not be read.\n\n` +
            `  Reason: ${reason}\n\n` +
...
export async function collectAddonsFromNodeModules(options) {
...
async function walkNodeModules(nodeModulesDir, addons, diagnostics, isRoot) {
    let entries;
    try {
        ...
        if (isRoot)
            throw new InvalidNodeModulesSourceError(nodeModulesDir, error);
        return;
    }
    for (const entry of entries) {
        ...
        await visitPackageDirectory(path.join(nodeModulesDir, entry.name), entry.name, addons, diagnostics);
    }
}
async function walkScopeDirectory(scopeDir, addons, diagnostics) {
    ...
    for (const entry of entries) {
        ...
        if (!isPackageEntry(entry))
            continue;
        await visitPackageDirectory(path.join(scopeDir, entry.name), `${scope}/${entry.name}`, addons, diagnostics);
    }
}
function isPackageEntry(entry) {
    return entry.isDirectory() || entry.isSymbolicLink();
}
async function visitPackageDirectory(packageDir, packageName, addons, diagnostics) {
