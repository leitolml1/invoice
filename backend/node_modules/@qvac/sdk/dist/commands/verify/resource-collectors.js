...
import { collectAddonsFromBundle } from '../../commands/verify/bundle-source.js';
import { listBarePrebuildFiles } from '../../commands/verify/prebuilds.js';
export const RESOURCE_COLLECTOR_PACKAGES = ['bare-cpu-info', 'bare-gpu-info'];
export const RESOURCE_COLLECTOR_SIZE_BUDGETS = {
    compressedBytes: 2 * 1024 * 1024,
    uncompressedBytes: 5 * 1024 * 1024
};
const DEFAULT_MEASUREMENT_DEPENDENCIES = {
    writeArchiveEntry
};
export async function acceptResourceCollectorPackaging(options) {
    const addons = await collectAddonsFromBundle({
        bundlePath: options.bundlePath,
        projectRoot: options.projectRoot
    });
    const collectors = new Map(addons
    ...
    const issues = [];
    const entriesByHost = new Map(options.hosts.map((host) => [host, []]));
    const budgets = options.budgets ?? RESOURCE_COLLECTOR_SIZE_BUDGETS;
    if (manifestResult.issue !== undefined)
        ...
        for (const host of options.hosts) {
            ...
            if (files.length === 0) {
                issues.push({
                    ...
                    package: packageName,
                    host,
                    message: `${packageName} is missing a prebuild for ${host} ` +
    ...
    for (const host of options.hosts) {
        ...
        try {
            ...
            issues.push(measurementFailedIssue(entries[0]?.package ?? RESOURCE_COLLECTOR_PACKAGES[0], host, error));
        }
    }
    return {
        ok: issues.length === 0,
...
export function formatResourceCollectorAcceptanceReport(report) {
    const lines = [
        `Resource collector packaging: ${report.ok ? 'PASS' : 'FAIL'}`,
        ...report.targets.map((target) => `${target.host}: ${target.compressedBytes} compressed / ` +
    ...
    for (const issue of report.issues) {
        const details = [issue.code];
        if ('host' in issue)
            details.push(`host: ${issue.host}`);
        if ('package' in issue)
            details.push(`package: ${issue.package}`);
        if ('actualBytes' in issue)
            details.push(`actual: ${issue.actualBytes} bytes`);
        if ('budgetBytes' in issue)
            details.push(`limit: ${issue.budgetBytes} bytes`);
...
export async function measureTarget(host, entries, dependencies = DEFAULT_MEASUREMENT_DEPENDENCIES) {
...
async function writeArchiveEntry(pack, entry) {
...
function applyBudgets(target, budgets, issues) {
    if (target.compressedBytes > budgets.compressedBytes) {
        issues.push({
            ...
            message: `${target.host} resource collectors use ${target.compressedBytes} compressed bytes ` +
    ...
    if (target.uncompressedBytes > budgets.uncompressedBytes) {
        issues.push({
            ...
            message: `${target.host} resource collectors use ${target.uncompressedBytes} uncompressed bytes ` +
                `(limit: ${budgets.uncompressedBytes}).`
        });
    }
}
function measurementFailedIssue(packageName, host, cause) {
...
function isResourceCollectorPackage(packageName) {
    return RESOURCE_COLLECTOR_PACKAGES.some((collector) => collector === packageName);
}
async function readAddonsManifest(manifestPath) {
    let parsed;
    try {
...
function isAddonsManifest(value) {
...
function invalidManifestIssue(manifestPath, cause) {
    const reason = cause instanceof Error ? cause.message : String(cause);
    return {
