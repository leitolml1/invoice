...
import { bundleSdk } from '../commands/bundle/index.js';
import { acceptResourceCollectorPackaging, formatResourceCollectorAcceptanceReport } from '../commands/verify/resource-collectors.js';
import { RequestValidationFailedError } from '../utils/errors-client.js';
const DESKTOP_HOSTS = [
    'darwin-arm64',
    'darwin-x64',
    'linux-arm64',
...
export function parseResourceCollectorCheckArgs(args) {
    const hosts = [];
...
export function resolveDesktopHost(platform, arch) {
    return validateDesktopHost(`${platform}-${arch}`);
}
export async function runResourceCollectorPackagingCheck(options, dependencies = DEFAULT_DEPENDENCIES) {
    const hosts = options.hosts.length > 0
        ? options.hosts
...
export async function runResourceCollectorPackagingCli(args, options = {}) {
...
function validateDesktopHost(host) {
    const desktopHost = DESKTOP_HOSTS.find((candidate) => candidate === host);
    if (desktopHost === undefined) {
        throw new RequestValidationFailedError(`Unsupported host "${host}". Expected one of: ${DESKTOP_HOSTS.join(', ')}`);
    }
    return desktopHost;
}
async function createTemporaryProject(sdkPackageRoot) {
    const projectRoot = await fsp.mkdtemp(path.join(os.tmpdir(), 'qvac-resource-collectors-packaging-'));
    try {
        await fsp.symlink(path.join(sdkPackageRoot, 'node_modules'), path.join(projectRoot, 'node_modules'), process.platform === 'win32' ? 'junction' : 'dir');
        return projectRoot;
    }
    catch (error) {
