...
import { requestSchema, responseSchema } from '../../schemas/common.js';
import { methodShapes } from '../../server/rpc/method-shapes.js';
import { constantsRegistry } from '../../schemas/constants-registry.js';
import { buildModelsRegistry } from './build-models-registry.js';
import { buildModelTypeMaps } from './build-model-type-maps.js';
import { buildErrorCodes } from './build-error-codes.js';
...
function wireTypesOf(schema) {
    if (schema instanceof z.ZodObject) {
        ...
        if (typeField instanceof z.ZodLiteral) {
            return Array.from(typeField.values).filter(function (value) {
    ...
    if (schema instanceof z.ZodUnion) {
        const nested = schema.options.flatMap(function (option) {
            return wireTypesOf(option);
        });
        return [...new Set(nested)];
    }
    if (schema instanceof z.ZodPipe) {
        return wireTypesOf(schema.in);
    }
    if (schema instanceof z.ZodIntersection) {
        const nested = [
            ...wireTypesOf(schema.def.left),
            ...wireTypesOf(schema.def.right)
        ];
        return [...new Set(nested)];
    }
    return [];
}
function wireTypeOf(schema, context) {
    const types = wireTypesOf(schema);
    if (types.length !== 1) {
        throw new Error(`Expected exactly one wire "type" literal for ${context}, found: [${types.join(', ')}]`);
    }
    return types[0];
}
function assertNoRefs(value, defName) {
    if (Array.isArray(value)) {
        for (const item of value)
            assertNoRefs(item, defName);
        return;
    }
    if (value !== null && typeof value === 'object') {
        for (const [key, child] of Object.entries(value)) {
            if (key === '$ref') {
                throw new Error(`Schema for "${defName}" emitted a $ref (${String(child)}); ` +
...
function mergeObjectSchemaParts(parts, defName) {
    const properties = {};
    const required = [];
    for (const part of parts) {
        ...
        for (const [key, value] of Object.entries(part['properties'] ?? {})) {
            if (key in properties && JSON.stringify(properties[key]) !== JSON.stringify(value)) {
                throw new Error(`Conflicting definitions for property "${key}" while flattening "${defName}"`);
            }
 ...
 */
function flattenAllOfWithUnion(json, defName) {
    const allOf = json['allOf'];
    if (!Array.isArray(allOf))
        return json;
    const isUnionMember = function (member) {
        return (member !== null &&
            typeof member === 'object' &&
            (Array.isArray(member['oneOf']) ||
                Array.isArray(member['anyOf'])));
    };
    const unionMembers = allOf.filter(isUnionMember);
    const plainMembers = allOf.filter(function (member) {
        return !isUnionMember(member);
    });
    if (unionMembers.length !== 1) {
        throw new Error(`Schema for "${defName}" uses allOf with ${unionMembers.length} union branches; ` +
    ...
    const arms = unionMember[unionKey];
...
function collapseSingleMemberUnions(node) {
    ...
    const wrapperRest = { ...node };
    delete wrapperRest[unionKey];
    if (!isSchemaObject(arm))
        return wrapperTitle ? wrapperRest : arm;
    const merged = { ...wrapperRest, ...arm };
    if (wrapperTitle)
        merged['title'] = wrapperTitle;
    return merged;
}
function constValueOf(arm, key) {
 ...
 */
function pickDiscriminatorKey(arms) {
...
function ensureUniqueTitle(base, seenTitles) {
    if (!seenTitles.has(base)) {
        seenTitles.add(base);
        return base;
    }
    let suffix = 2;
    while (seenTitles.has(`${base}${suffix}`))
        suffix++;
 ...
 */
function nameUnionArms(arms, namePrefix) {
    if (arms.length <= 1) {
    ...
    const names = new Array(arms.length);
    for (const value of groupOrder) {
        ...
        if (indexes.length === 1) {
            names[indexes[0]] = groupPrefix;
            continue;
        }
        const subNames = nameUnionArms(indexes.map(function (index) {
            return arms[index];
        }), groupPrefix);
        indexes.forEach(function (armIndex, i) {
            names[armIndex] = subNames[i];
        });
    }
    return names;
}
 ...
 */
function titleSchemaNode(node, namePrefix, seenTitles) {
    if (!isSchemaObject(node))
        return;
    const existingTitle = typeof node['title'] === 'string' ? node['title'] : undefined;
    if (existingTitle)
        seenTitles.add(existingTitle);
    ...
    if (arms) {
        ...
        arms.forEach(function (arm, index) {
            titleSchemaNode(arm, armNames[index], seenTitles);
        });
        return;
    }
    ...
    const assignedTitle = needsOwnTitle(node) && !existingTitle ? ensureUniqueTitle(namePrefix, seenTitles) : undefined;
    if (assignedTitle)
        node['title'] = assignedTitle;
...
function titleNestedSchemas(root, rootTitle, seenTitles) {
...
export function toWireJsonSchema(schema, io, defName) {
    ...
    const collapsed = collapseSingleMemberUnions(flattened);
...
function classTitleFor(name, suffix) {
...
const progressByMethod = {
    ...
    rag: {
        condition: "request.withProgress === true && ['ingest', 'saveEmbeddings', 'reindex'].includes(request.operation)",
        responseType: 'rag:progress'
    },
    finetune: {
        condition: "request.withProgress === true && ['start', 'resume', undefined].includes(request.operation)",
        responseType: 'finetune:progress'
    }
};
export function buildContract() {
    const methodNames = Object.keys(methodShapes).sort();
    ...
    for (const wireType of requestByType.keys()) {
        if (!methodNames.includes(wireType)) {
    ...
    for (const [methodName, progress] of Object.entries(progressByMethod)) {
        if (!responseByType.has(progress.responseType)) {
            throw new Error(`progressByMethod["${methodName}"] points at response type "${progress.responseType}", ` +
    ...
    const constantVarNames = new Map();
    ...
    const schemaDocument = {
        ...
        description: 'Generated from the SDK Zod schemas by `bun run contract:export`. Do not edit by hand. ' +
...
async function formatJson(value, fileName) {
