import { toWireJsonSchema } from '../../scripts/contract/build-contract.js';
/**
 ...
 */
export function buildPluginContract(plugin) {
    const handlers = Object.entries(plugin.handlers).map(function ([name, handler]) {
        return {
            name,
            streaming: handler.streaming === true,
            requestSchema: toWireJsonSchema(handler.requestSchema, 'input', `${plugin.modelType}.${name}.request`),
            responseSchema: toWireJsonSchema(handler.responseSchema, 'output', `${plugin.modelType}.${name}.response`)
        };
    });
